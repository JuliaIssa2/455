...
def encrypt_text_ecb(plaintext: str, user_key: str) -> str:
    """
    Encrypt arbitrary-length text using AES-128 in ECB mode,
    built on top of your encrypt_block() (single-block AES).

    Returns:
        full_cipher_hex: C1 || C2 || ... as one big hex string (uppercase)
    """
    # 1) Normal text -> bytes
    pt_bytes = plaintext.encode("utf-8")
    ...
    # 4) ECB over blocks (no IV, no XOR)
    cipher_blocks = []
    ...
    for i in range(0, len(pt_padded), BLOCK_SIZE):
        ...
        c_hex, _, _ = encrypt_block(block_hex, key_hex)  # only need ciphertext
        cipher_blocks.append(c_hex)

    full_cipher_hex = "".join(cipher_blocks).upper()
    return full_cipher_hex


# -------- AES-ECB decryption back to NORMAL TEXT --------

def decrypt_text_ecb(full_cipher_hex: str, user_key: str) -> str:
    """
    Decrypt ciphertext produced by encrypt_text_ecb and return the original text.

    full_cipher_hex is: C1 || C2 || ... (all uppercase hex)
    """
    if len(full_cipher_hex) == 0 or len(full_cipher_hex) % 32 != 0:
        raise ValueError(
            "Ciphertext hex length must be non-empty and a multiple of 32 "
            "(16 bytes per block)."
        )
    ...
    for i in range(0, len(full_cipher_hex), 32):
        c_hex = full_cipher_hex[i : i + 32]
        p_hex, _, _ = decrypt_block(c_hex, key_hex)
        pt_padded_bytes += bytes.fromhex(p_hex)
